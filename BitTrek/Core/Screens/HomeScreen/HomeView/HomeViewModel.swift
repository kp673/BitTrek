...
class HomeViewModel: ObservableObject {
    ...
    func getPortfolioData() {
        ...
        allCoins.indices.forEach { index in
            guard
                let entity = savedEntity.first(where: {
                    $0.coinId == allCoins[index].id
                })
            else { return }
    ...
    func saveButtonPressed() {
        ...
        portfolioDataService.updatePortfolio(
            coin: coin, amount: Double(quantity) ?? 0)
    ...
    func updateStatistics() {
        ...
        let portfolio = Statistics(
            ...
            value: portfolioValue.asCurrencyUpto2Places(),
    ...
    private func fetchCoins() async throws -> Coins? {
        ...
        do {
            let coins: Coins? = try await DataService.shared.get(
                url: url, headers: headers, queryComponents: queryComponents)
            return coins
        } catch {
            throw error
        }
    ...
    private func fetchMarketData() async throws -> GlobalData? {
        ...
        do {
            let globalData: GlobalData? = try await DataService.shared.get(
