...
final class DataService: Sendable {
    ...
    private func get<T: Codable>(urlComponents: URLComponents, headers: [String: String]? = [:]) async throws -> T {
        ...
        guard let response = response as? HTTPURLResponse, response.statusCode == 200 else { throw CoinError.invalidResponse }
